' Transpiled on 19-11-2022 16:27:11
...
Function highscr.get_input%(ctrl$)
  ...
  If key% = 0 Then
    Call ctrl$, key%
    Select Case key%
      Case ctrl.A     : highscr.get_input% = &h01 ' Magic value to use for fire
...
Sub menu.update_controller(idx%, delta%, ui_ctrl$)
  ...
  Do
    ...
    If ok% Then
      If CTRL_DRIVER$(i%) <> ui_ctrl$ Then
        ok% = menu.can_open_controller%(CTRL_DRIVER$(i%))
      EndIf
    EndIf
  Loop Until ok%
